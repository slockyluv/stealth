 ...
 */
export function buildCustomId(scope, action, ...args) {
    ...
    const id = parts.join(':');
    if (id.length > 100)
        throw new Error(`customId too long (${id.length}). Must be <= 100.`);
    return id;
}
export function parseCustomId(customId) {
    ...
    if (!scope || !action)
        return null;
    if (!SAFE_PART.test(scope) || !SAFE_PART.test(action))
        return null;
    for (const a of args) {
        if (!a || !SAFE_PART.test(a))
            return null;
    }
    return { scope, action, args };
}
export function customIdKey(parts) {
